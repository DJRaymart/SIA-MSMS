...
<head>
    <title>Admin Terminal | ICT Office</title>
    ...
    <style>
        ...
        /* Floating modal - AVR style (all ICT modals) */
        .ict-modal-overlay {
            backdrop-filter: blur(6px);
            animation: ict-modal-fadeIn 0.25s ease-out;
            z-index: 9999;
            padding: 1rem;
            box-sizing: border-box;
        }
        .ict-modal-content {
            animation: ict-modal-floatIn 0.4s cubic-bezier(0.34, 1.56, 0.64, 1);
            box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25), 0 0 0 1px rgba(0, 0, 0, 0.05);
            width: 100%;
            max-width: min(24rem, calc(100vw - 2rem));
            flex-shrink: 0;
        }
        @keyframes ict-modal-fadeIn {
            from { opacity: 0; }
            to { opacity: 1; }
        }
        @keyframes ict-modal-floatIn {
            from {
                opacity: 0;
...
<body class="bg-slate-50">
    <script>
    window.ICT_API = <?php echo json_encode($ict_api ?? $baseS . 'ictOffice/api'); ?>;
    window.ICT_PUBLIC = <?php echo json_encode($ict_public ?? $baseS . 'ictOffice/public'); ?>;
    </script>
